namespace ClientV3
{
    ...
    class App
    {
        ...
        /// </summary>
        static readonly string BucketKey = string.Format("{0}_{1}", "aumadbuckets",DateTime.Now.ToString("yyyyMMddhhmmss"));
        ...
        private async Task<string> SetupActivityAsync(string myApp)
        {
            ...
            var activity = new Activity()
            {
                ...
                Parameters = new Dictionary<string, Parameter>()
                    {
                        { "inputFile", new Parameter() { Verb= Verb.Get,
                            LocalName = "$(HostDwg)",  Required = true } },
                        { "inputZip", new Parameter() { Verb= Verb.Get, 
                            Zip=true, LocalName = "export", Required = true} },
                        { "Result", new Parameter() { Verb= Verb.Put,
        ...
        private async Task<bool> SetupOwnerAsync()
        {
            ...
            {
                Console.WriteLine($"\tnickname for this clientId Exists {nickname}");
                return true;
            }
            ...
            var response = await resp.EnsureSuccessStatusCodeAsync();
